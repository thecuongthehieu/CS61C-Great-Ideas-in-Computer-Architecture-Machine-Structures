...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        int is_lable = add_if_label(input_line, token, byte_offset, symtbl);
        if (is_lable != 0) {
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        // go to the next line.
        char *inst_name = strtok(buf, IGNORE_CHARS);
        if (buf == NULL) {
            continue;
        }
        ...
        if (translate_inst(output, inst_name, args, num_args, byte_offset, symtbl, reltbl) == -1) {
            error = 1;
            raise_inst_error(input_line, inst_name, args, num_args);
        }
        byte_offset += 4;
